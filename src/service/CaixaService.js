...
class Caixa {
    async #getCaixa(search, page, year) {
        return axios.get(`${API_ENDPOINT}/caixa/getCaixas.json?page=${page}&limit=8&active=true&year=${year}&search=${search}`, this.buildAuthHeader())
    }
    ...
    getCaixas(disable, page, search, year, onFetch) {
        if (disable) {
            this.#getCaixaInactive(search, page, year)
                .then(response => onFetch(this.#formatDate(response.data)))
        } else {
            this.#getCaixa(search, page, year)
                .then(response => onFetch(this.#formatDate(response.data)))
        }
    }
